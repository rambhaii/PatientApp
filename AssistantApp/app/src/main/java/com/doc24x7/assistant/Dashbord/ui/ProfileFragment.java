...
import com.doc24x7.assistant.DoctorDashboad.QualificationWithoutDeleteAdapter;
...
RecyclerView rv_qualification;
    ...
    TextView Name, email, Phoneno, qualification, experience, spetiality, address,updateQuaification,updateAccDetail;
    TextView beneficiaryname
            , bankname,branchname,accountnumber,ifsc,upi;
    @Override
    ...
    private void Getid(View v) {
        ...
        spetiality = v.findViewById(R.id.spetiality);
    ...
    private void SetData() {
        ...
        bankname.setText("Bank Name        : "+(balanceCheckResponse.getBank_name()==null?" ":balanceCheckResponse.getBank_name().replace('"', ' ')));
        accountnumber.setText("AccountNo         : " + (balanceCheckResponse.getAccount_no()==null?" ":balanceCheckResponse.getAccount_no().replace('"', ' ')));
        ifsc.setText("IFSC Code          : " + (balanceCheckResponse.getIfsc_code()==null?" ":balanceCheckResponse.getIfsc_code().replace('"', ' ')));
        branchname.setText("Branch Name     : " + (balanceCheckResponse.getBranch_name()==null?" ":balanceCheckResponse.getBranch_name().replace('"', ' ')));
        upi.setText("UPI                      : " + (balanceCheckResponse.getUpi()==null?" ":balanceCheckResponse.getUpi().replace('"', ' ')));
        beneficiaryname.setText("Account Holder : " + (balanceCheckResponse.getAccount_holder_name()==null?" ":balanceCheckResponse.getAccount_holder_name().replace('"', ' ')));
    }
