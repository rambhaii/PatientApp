package com.doc24x7.doctor.RTM.activity;
...
import com.doc24x7.doctor.MainActivity;
import com.doc24x7.doctor.R;
import com.doc24x7.doctor.RTM.MessageAdapter;
import com.doc24x7.doctor.RTM.model.MessageBean;
import com.doc24x7.doctor.RTM.model.MessageListBean;
import com.doc24x7.doctor.RTM.rtmtutorial.ChatManager;
import com.doc24x7.doctor.RTM.utils.ImageUtil;
    ...
    private void init() {
        ...
        if (mIsPeerToPeerMode) {
          ...
          //  mTitleTextView.setText(MessageFormat.format("{0}({1})", mChannelName, mChannelMemberCount));
            mTitleTextView.setText(targetName);
            createAndJoinChannel();
        }

        LinearLayoutManager layoutManager = new LinearLayoutManager(this);
        layoutManager.setOrientation(RecyclerView.VERTICAL);
        mMessageAdapter = new MessageAdapter(this, mMessageBeanList, new MessageAdapter.OnItemClickListener() {
            @Override
            public void onItemClick( MessageBean message) {
                if (message.getMessage().getMessageType() == RtmMessageType.IMAGE) {
                    if (!TextUtils.isEmpty(message.getCacheFile())) {
                        Glide.with(MessageActivity.this).load(message.getCacheFile()).into(mBigImage);
                        mBigImage.setVisibility(View.VISIBLE);
                    } else {
                        ImageUtil.cacheImage(MessageActivity.this, mRtmClient, (RtmImageMessage) message.getMessage(), new ResultCallback<String>() {
                            @Override
    ...
    private void sendPeerMessage(final RtmMessage message) {
        mRtmClient.sendMessageToPeer(mPeerId, message, mChatManager.getSendMessageOptions(), new ResultCallback<Void>() {
            ...
            public void onFailure(ErrorInfo errorInfo) {
               ...
               // Toast.makeText(MessageActivity.this, ""+errorCode, Toast.LENGTH_SHORT).show();
                runOnUiThread(() -> {
                    switch (errorCode) {
