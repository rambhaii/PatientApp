...
import com.doc24x7.doctor.Video.connectionservice.OpenDuoConnectionService;
...
public abstract class BaseCallActivity extends BaseRtcActivity implements RtmChannelListener, ResultCallback<Void> {
    ...
    public void gotoCallingInterface(String peerUid, String channel, int role) {
        if (new Config(getApplicationContext()).useSystemCallInterface()) {
            // Not supported yet.
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            try {
                ...
                PhoneAccount pa = telecomManager.getPhoneAccount(
                        new Config(getApplicationContext()).getPhoneAccountOut().getAccountHandle());
    ...
    protected void cancelLocalInvitation() {
        if (mRtmCallManager != null &&  new Global().getLocalInvitation() != null) {
            mRtmCallManager.cancelLocalInvitation( new Global().getLocalInvitation(), this);
        }
    }
